...
log_daemon_msg "Downloading GFW IP list" "$GFWLIST "
...
curl -4sSkL "$url" | $base64_decode | { perl -pe "$gfwlist_txt_perl_script_string"; gfwlist_txt_append_domain_names; } | sort | uniq > $GFWLIST
log_end_msg $?

log_daemon_msg "Downloading China IPv4 route list" "$CHNROUTE "
curl -4sSkL 'http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest' | grep CN | grep ipv4 | awk -F'|' '{printf("%s/%d\n", $4, 32-log($5)/log(2))}' > $CHNROUTE
log_end_msg $?

log_daemon_msg "Downloading China IPv6 route list" "$CHNROUTE6 "
